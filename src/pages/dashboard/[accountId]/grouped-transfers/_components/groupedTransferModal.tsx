import {
    ...
    ModalHeader,
    ModalOverlay, NumberInput, NumberInputField, Tag, TagLabel, useToast,
} from "@chakra-ui/react";
import { Select } from "chakra-react-select";
import { useMutation, useQuery, useQueryClient } from "@tanstack/react-query";
import { getAllAccounts } from "@/api/account.ts";
import { doGroupTransfer, NewGroupedTransfer } from "@/api/groupedTransaction.ts";
...
export default function GroupedTransferModal(props: GroupedTransferModalProps) {
    ...
    const mutation = useMutation({
        ...
        onSettled: () => {
            ...
            invalidKeys.forEach(key => queryClient.invalidateQueries({
                queryKey: [key]
            }))
        },
        onSuccess: () => {
    ...
    const { register, handleSubmit, formState, control} = useForm<NewGroupedTransfer>();
            ...
            <ModalBody>
                <Flex direction={"column"}>
                    <FormControl>
                        ...
                        <Input placeholder={"Eg: Salary, Tickets ,..."} {...register("label", {required: true})} />
                    </FormControl>
                    <FormControl>
                            ...
                            <Controller render={({
                                ...
                                <Select
                                    ...
                                    options={data?.map(res => ({
                                        ...
                                        value: res.id,
                                    })).filter(data => data.value !== props.accountId)}
                                    onChange={onChange}
                                />
                            )}
                                        name={"targetsId"}
                    ...
                    <FormControl>
                        ...
                        <Input type={"date"} {...register("effectDate", {required: true})}/>
                    </FormControl>
                </Flex>
            </ModalBody>
            <ModalFooter gap={2}>
