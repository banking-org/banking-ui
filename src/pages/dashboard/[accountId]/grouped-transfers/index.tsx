...
export default function GroupedTransfers() {
    ...
    return <Flex p={8} direction={"column"}>
        ...
        <Flex>
            {
                data?.length === 0 ? <Flex w={"100%"} justifyContent={"center"} alignItems={"center"}>
                    ...
                    <Table>
                        ...
                        <Tbody>
                            {
                                data?.map((transfer, id) => (<Tr key={id}>
                                    <Td>{transfer.id}</Td>
                                    <Td>{transfer.label}</Td>
                                    <Td>{format(transfer.updatedAt, "yyyy-MM-dd")}</Td>
                                </Tr>))
                            }
                        </Tbody>
                    </Table>
        ...
        <GroupedTransferModal isOpen={isOpen} onClose={onClose} accountId={+accountId}/>
    </Flex>;
}